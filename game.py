...
class Game(object):
    ...
    def graphic(self, board, player1, player2):
        ...
        window = tkinter.Tk()
        bg_color = '#%02x%02x%02x' % (226, 188, 136)  # Convert RGB to hex

        self.cv = tkinter.Canvas(window, height=height*30+100, width=width*30 + 30, bg = bg_color)

        board_color = '#%02x%02x%02x' % (166, 136, 100)  # Convert RGB to hex

        self.cv.create_rectangle(Point(0, 0).pixel_x, Point(0, 0).pixel_y,
                                 Point(width - 1, height - 1).pixel_x,
                                 Point(width - 1, height - 1).pixel_y, fill=board_color)
        ...
        for i in range(width):  #vertical line
            if i > 0:
                self.cv.create_text(self.chess_board_points[i][height - 1].pixel_x,
                                    self.chess_board_points[i][height - 1].pixel_y+kw_size,
                                    text=str(i), fill='black', anchor='center')
        ...
        for j in range(height):  #rizontal line
            if j != height-1:
                self.cv.create_text(self.chess_board_points[0][j].pixel_x - kw_size,
                                    self.chess_board_points[0][j].pixel_y,
                                    text=str(height-j-1), fill='black', anchor='center')
        ...
        self.cv.create_text(self.chess_board_points[0][height - 1].pixel_x - kw_size,
                            self.chess_board_points[0][height - 1].pixel_y + kw_size,
                            text=str(0), fill='black', anchor='center')

        self.button = tkinter.Button(window, text="start game!", command=self.run)
        self.button.place(x=(Point(width-1, 0).pixel_x+Point(0, 0).pixel_x)/2, y=height*30+70, anchor='center')
